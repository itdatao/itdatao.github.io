...
并不是所有的返回类型都能受益于optional的返回类型。容器类型，包括Collection，map，stream，数组，以及optional都不应该再用optional进行包装了。你应该放回一个空的List，而不是空的Optional<List>。
...
本地方法移植性较差，更难调试，如果使用不当回造成内存泄漏，并且代码可读性不是很好。
